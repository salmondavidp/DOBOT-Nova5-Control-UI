let ws = null;
        ...
        function updateStatus(status) {
            ...
            if (hasFault) {
                ...
                if (window.lastFaultKey !== faultKey) {
                    ...
                    log(`⚠️ FAULT detected on slave(s): ${faultList}`, 'err');
                    templateLog(`⚠️ FAULT on slave(s): ${faultList}`, 'error');
        ...
        function handleEvent(event) {
            ...
            if (event.type === 1) {
                ...
                log(`⚠️ ${event.message}`, 'wrn');
                templateLog(`⚠️ ${event.message}`, 'warning');
                ...
                templateLog(`❌ ${event.message}`, 'error');
            ...
            const alertHtml = `
                ...
                ">
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                        <span style="font-size: 1.5rem;">${type === 'error' ? '❌' : type === 'warning' ? '⚠️' : 'ℹ️'}</span>
        ...
        function updatePositionTable(status) {
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                const movingIndicator = isMoving ? '<span style="color: var(--accent-blue); animation: pulse 1s infinite;">●</span> ' : '';
        ...
        function handleResponse(resp) {
            ...
            if (resp.success) {
                ...
                if (resp.message.includes('Running template')) {
                    templateLog('▶ ' + resp.message, 'config');
                ...
                if (resp.message.includes('Template') || resp.message.includes('template') || 
                    ...
                    templateLog('✗ ' + resp.message, 'error');
                }
            }
            ...
            if (resp.data && resp.data.template_start) {
                ...
                templateLog('▶ Template started', 'config');
            }
            ...
            if (resp.data && resp.data.template_complete) {
                ...
                if (total) {
                    templateLog(`✓ Template complete (Total: ${total.toFixed(1)}s)`, 'complete');
                }
            }

            if (resp.data && resp.data.template_move) {
                const move = resp.data.template_move;
                templateLog(`  → Slave ${move.slave}: ${move.position}`, 'move');
            }
        ...
        function updateTemplateDisplay(template, positions) {
            ...
            template.steps.forEach((step, i) => {
                ...
                html += `<tr id="step-row-${i + 1}">
                    ...
                    <td id="step-order-${i + 1}" style="color: var(--text-secondary); font-size: 0.75rem;">—</td>
                </tr>`;
            });
        ...
        function updateStepMoveOrder(stepIndex, moveOrder, isSpreading) {
            ...
            // Format: "0→1→2→3" with spreading/converging indicator
            const orderStr = moveOrder.map(s => s + 1).join('→');
            const indicator = isSpreading ? '↔' : '→←';  // Spreading or converging symbol
        ...
        function addOscLogEntry(entry) {
            ...
            if (entry.type === 'recv') {
                div.innerHTML = `${timeSpan} <span style="color: var(--accent-blue);">◀ RECV:</span> ${entry.message}`;
            } else if (entry.type === 'send') {
                div.innerHTML = `${timeSpan} <span style="color: var(--accent-green);">▶ SEND:</span> ${entry.message}`;
            } else {
